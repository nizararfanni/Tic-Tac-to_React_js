...
const TicTacTo = () => {
  ...
  const won = (winner) => {
    setLock(true);
    if (winner === "x") {
      tittleReff.current.innerHTML = `Congratulations <img src=${cross_icon}> wins`;
    } else {
  ...
  const reset = () => {
    setLock(true);
    data = ["", "", "", "", "", "", "", "", ""];
    tittleReff.current.innerHTML = `tic tac to game in <span>React</span>`;
    box_array.map((e) => {
      e.current.innerHTML = "";
    });
  };
  ...
  return (
    <div className="container">
      <h1 className="tittle" ref={tittleReff}>
        {" "}
        tic tac to game in <span>React</span>
      </h1>
      <div className="board">
        <div className="row1">
